...
writing guest memory to disk. The same mechanism must work on first boot, when
there is no page-fault handler, and must be usable as the source side of a live
migration (dirty ranges while running, consistent final pass while paused).
...
- The virtio-mem hotplug region is currently allocated separately
  (`VmResources::allocate_memory_region`, from `build_microvm_for_boot`),
  which in the memfd case yields a second memfd. Only its guest address needs
  the KVM VM to exist; its size is known up front.
...
- The UFFD protocol (`src/vmm/src/persist.rs`, `guest_memory_from_uffd`,
  ...
  the stream. The connection therefore already stays open for the VM's life
  (the handler uses it to notice Firecracker exiting), but Firecracker never
  reads it. The example handler runtime
...
columns in the mapping table, Firecracker reading requests on the socket, and
a way to run the handshake at boot.
...
- `VmResources` owns the memfd (`Option<Arc<File>>` plus a next-offset
  cursor, or a small allocator object returned by `allocate_guest_memory` and
  reused by `allocate_memory_region`). The memfd is sized
...
- Out of scope: `GuestRegionMmapExt::discard_range` (balloon, virtio-mem
  ...
  a shared mapping. This limitation is inherited from the vhost-user memfd path
  and is a separate workstream. It does not affect snapshot identity:
  Firecracker and the backend read the same memfd, and unplugged slots are
  skipped by both based on `plugged` state, not on page contents.
- Shared mappings have costlier page faults than anonymous memory, and THP on
  shmem depends on the host's `shmem_enabled`. This is the price of sharing,
  as for vhost-user today.
...
- UFFD protocol (existing, unchanged, kept indefinitely): a bare JSON array of
  ...
  traffic on the socket. Selected with `backend_type: Uffd`. No existing
...
protocol or from a file; a VM restored with the UFFD protocol can be
snapshotted and restored with a memory backend. Consequences of the choice are
confined to the running instance: memory backing, handshake format, and how
`snapshot/create` writes memory.

### 3. Handshake (memory backend protocol)

Firecracker connects to the backend's UDS and sends one message and the fds in
a single `sendmsg`:
...
- The backend's role is determined by `fds`: if `uffd` is present it registers
  a fault-serving path; it always keeps the memfd and always serves requests.
...
(`src/vmm/src/lib.rs`), which gives the handler `&mut Vmm`. Requests are
handled synchronously, one at a time. There is a single request:
...
- `{"GetDirtyRanges": {}}` → callable in any VM state, consuming. Firecracker
  ...
  merged, page-aligned ranges in memfd/file offset space. If the reply cannot
  be sent, the bitmap is folded back with `store_dirty_bitmap` so the next call
...
Dirty-bitmap ownership: with a memory backend connected, Firecracker only
reads or resets the bitmaps when handing ranges to the backend.
...
this is a memory-allocation property like `track_dirty_pages` and
`huge_pages`. It is accepted from the JSON config file as well. Not persisted
in the snapshot.
...
- Populating from a file while sharing memory is not offered for now, so a
  memory backend at restore always receives a uffd. A MAP_PRIVATE mapping of
  the snapshot file cannot be handed out as an fd, and shmem cannot lazily
  reflect another file. If the need arises the options are, in order of
  preference: an eager copy of the snapshot file into the memfd
  (`copy_file_range` file→shmem, cross-filesystem on 5.10, with a read/write
  fallback) at the cost of reading and allocating all guest memory at restore;
  an in-Firecracker UFFD fault-handler thread copying pages on demand; or a
  MAP_SHARED mapping of a reflinked copy of the snapshot file, which is lazy
  but makes every guest write dirty page cache subject to writeback. None of
  them changes the handshake, so the decision can be deferred.
...
`mem_backend.backend_type` must be `MemoryBackend`; without one,
`MemoryBackend` is rejected and behaviour is unchanged.

Synchronisation is required because `Paused` only stops vCPUs
(`Vmm::pause_vm`); device emulation on the VMM thread keeps running and can
write guest memory and used rings (a tap RX frame, an in-flight block
completion) after the vmstate save. Firecracker's own snapshots avoid this only
because save and dump are one synchronous call. The backend path recreates
that property. On the VMM thread:

1. require `Paused`, as today;
2. save vmstate (`save_state` runs every device's `prepare_save`; virtio-block
   drains in-flight io_uring requests, so no kernel-side DMA into guest memory
   is pending afterwards);
3. compute the ranges: `Diff` → the `GetDirtyRanges` procedure (consuming);
   `Full` → all plugged slots;
4. send
   `{"SnapshotRequest": {"snapshot_type": "Diff", "regions": [...], "ranges": [...]}}`
   (regions carry the current `plugged` state);
5. block reading exactly one
   `{"SnapshotDone": {"success": true|false, "message": "..."}}`; any other
   message is a protocol error;
6. return 204, or an error carrying the backend's message. On failure, protocol
   error or disconnect, fold the ranges of step 3 back into Firecracker's
   bitmap so the next diff is a superset.
...
A handler implements one protocol, so the reference memory backend is a
separate example (`src/firecracker/examples/memory_backend/`) rather than a
mode of the UFFD handlers. It shares the fault-serving and runtime code with
...
- the handshake parser: JSON object with `fds`, keeps the memfd, builds the
  uffd path only if a uffd was received;
- message handling on the UDS in the same poll loop: issuing `GetDirtyRanges`
  for pre-copy passes and answering `SnapshotRequest` by copying ranges from
  the memfd into the target file with `copy_file_range` (creating at
  `total_size` for `Full`, merging for `Diff`, zeroing unplugged slots) and
  replying `SnapshotDone`;
...
1. Full snapshot layout: boot with a backend, run a workload, pause,
   ...
   a healthy guest.
2. Diff self-consistency (the backend-side analogue of
...
3. Chains: several diffs rebased onto the base equal a final full copy, and the
   rebased result restores with a healthy guest.
4. Cross-check against a Firecracker-made snapshot: restore one VM with a
...
5. Variants of 1–3: virtio-mem with unplugged slots, balloon inflated,
...
- Negative: `snapshot/create` with `MemoryBackend` but no backend → 400;
  ...
  `File`/`Uffd` → 400; backend disconnect → VM keeps running,
  `snapshot/create` errors; `SnapshotDone{success:false}` → error surfaced and
  the next diff includes the failed ranges.
...
- Metrics: `mem_backend.{handshake_fails, requests, request_fails, disconnects,
  snapshot_request_duration_us}`.
...
2. Memory backend handshake serialisation, `MachineConfig.mem_backend`,
   boot-time handshake, socket registered in `Vmm::init`, seccomp, metrics.
3. `GetDirtyRanges`, with the dirty-page iteration factored out of
   `GuestMemorySlot::dump_dirty` so both paths share one iterator; Rust
   identity tests.
4. `PUT /snapshot/create` with `MemoryBackend`: window, rejections, fold-back.
5. `snapshot/load` with `backend_type: MemoryBackend`: memfd + uffd on shmem,
   handshake with two fds.
6. Reference memory backend example (shared code with `examples/uffd`),
   Python framework, integration and compatibility tests.
7. Docs (`docs/snapshotting/memory-backend.md`, updates to
